...
func createCollection(db *nosql.Database) {
	...
	for {
		...
		fmt.Print("Voulez-vous créer un index sur ce champ? (oui/non): ")
		var indexChoice string
		fmt.Scanf("%s", &indexChoice)

		if indexChoice == "oui" || indexChoice == "o" || indexChoice == "y" || indexChoice == "yes" {
			fmt.Println("Type d'index:")
			fmt.Println("1. Index standard")
			fmt.Println("2. Index unique")
			fmt.Println("3. Index textuel (pour les champs de type texte)")

			fmt.Print("Choisissez un type d'index (1-3): ")
			var indexTypeChoice int
			fmt.Scanf("%d", &indexTypeChoice)

			switch indexTypeChoice {
			case 1:
				fmt.Printf("Index standard créé sur le champ '%s'\n", fieldName)
			case 2:
				fmt.Printf("Index unique créé sur le champ '%s'\n", fieldName)
			case 3:
				if fieldType == nosql.FieldTypeString {
					fmt.Printf("Index textuel créé sur le champ '%s'\n", fieldName)
				} else {
					fmt.Println("Les index textuels ne peuvent être créés que sur des champs de type texte.")
				}
			default:
				fmt.Println("Type d'index invalide.")
			}
		}
	}
...
func manageIndexes(db *nosql.Database) {
	collections := db.ListCollections()
	if len(collections) == 0 {
		fmt.Println("Aucune collection n'existe. Veuillez d'abord en créer une.")
		return
	...
	fmt.Println("1. Créer un nouvel index")
	fmt.Println("2. Lister les index existants")
	fmt.Println("3. Supprimer un index")
	fmt.Println("4. Retour au menu principal")

	fmt.Print("Choisissez une opération (1-4): ")
	var opChoice int
	fmt.Scanf("%d", &opChoice)
...
func createIndex(collection *nosql.Collection) {
	fields := collection.GetFields()
	if len(fields) == 0 {
		fmt.Println("Cette collection n'a pas de champs définis.")
		return
	...
	// Dans une implémentation réelle, nous utiliserions des méthodes comme:
	// collection.CreateIndex(field.Name)
	// collection.CreateUniqueIndex(field.Name)
	// collection.CreateTextIndex(field.Name)
	...
	case 2:
		fmt.Printf("Index unique créé sur le champ '%s'\n", field.Name)
	case 3:
		if field.Type == nosql.FieldTypeString {
			fmt.Printf("Index textuel créé sur le champ '%s'\n", field.Name)
		} else {
			fmt.Println("Les index textuels ne peuvent être créés que sur des champs de type texte.")
		}
...
func listIndexes(collection *nosql.Collection) {
	// Dans une implémentation réelle, nous utiliserions une méthode comme:
	// indexes := collection.GetIndexes()
	// Pour cet exemple, nous allons simuler la liste des index

	fmt.Println("Index existants sur cette collection:")
	fmt.Println("(Cette fonctionnalité serait implémentée dans une version réelle)")
	fmt.Println("Pour voir les index, il faudrait ajouter une méthode GetIndexes() à la collection.")
}

func deleteIndex(collection *nosql.Collection) {
	// Dans une implémentation réelle, nous utiliserions une méthode comme:
	// indexes := collection.GetIndexes()
	// Pour cet exemple, nous allons simuler la suppression d'un index

	fmt.Println("Index existants sur cette collection:")
	fmt.Println("(Cette fonctionnalité serait implémentée dans une version réelle)")
	fmt.Println("Pour supprimer un index, il faudrait ajouter une méthode DropIndex() à la collection.")

	fmt.Print("Nom de l'index à supprimer: ")
	var indexName string
	fmt.Scanf("%s", &indexName)

	// Dans une implémentation réelle: collection.DropIndex(indexName)
	fmt.Printf("Index '%s' supprimé avec succès!\n", indexName)
}
